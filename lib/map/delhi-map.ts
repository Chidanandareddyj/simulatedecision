// @ts-nocheck
// Pixel-art city renderer ported from simfrancisco map.js

import { COLORS, TIMING, MAP } from "./config";
import { makeThought, verdictReaction } from "./thoughts";
...
export interface MapSprite {
  ...
  ang: number; speed: number; turnClock: number;
  verdict: Verdict | null; activateAt: number;
  seed: number; thought: string; rationale: string | null;
  name?: string; age?: number; race?: string; educ?: string;
  hood?: string; values?: Record<string, number>; action?: string;
}

export class DelhiMap {
  ...
  landBox: { minX: number; minY: number; maxX: number; maxY: number } | null;
  sprite: HTMLImageElement;
  spriteReady: boolean;
  cam: { x: number; y: number; zoom: number };
  camTarget: { x: number; y: number; zoom: number };
  zoomedIn: boolean;
  onZoomChange: ((zoomedIn: boolean) => void) | null;
  onSpriteTap: ((sprite: MapSprite) => void) | null;
  onEmptyTap: (() => boolean) | null;
  mode: string;
  t0: number;
  lastT: number;
  revealT0: number;
  ...
  onRevealComplete: (() => void) | null;
  bubbleIdx: number[];
  bubbleT: number;
  populationRunId: string | null;
  chatterAsked: Set<number>;
  onNeedChatter: ((ids: number[]) => void) | null;
  _raf: number | null;
  cssW: number;
  cssH: number;
  ...
  setSim(_slug: string, populationRunId: string | null) {
    this.populationRunId = populationRunId || null;
    this.chatterAsked = new Set();
  }

  // Replace a resident's thought with its LLM-generated one (matched by backend id).
  setThought(id: number, text: string) {
    if (!text) return;
  ...
  _requestChatter() {
    if (!this.populationRunId || !this.onNeedChatter || !this.bubbleIdx.length) return;
    const ids = [];
  ...
  drawCharTo(canvas: HTMLCanvasElement, char: number) {
  ...
  startReveal(verdicts: Verdict[], durationMs = TIMING.revealMs) {
  ...
  setRationales(rationales: string[] | null | undefined) {
